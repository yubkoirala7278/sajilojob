...
class CompanyController extends Controller
{
    ...
    public function createCompany()
    {
        ...
        $packages = Package::select('id', DB::raw("CONCAT(`package_job`,', ' ,`package_title`, ', NPR', `package_price`, ', Days:', `package_num_days`, ', Listings:', `package_num_listings`) AS package_detail"))->where('package_for', 'like', 'employer')->pluck('package_detail', 'id')->toArray();
    ...
    public function storeCompany(CompanyFormRequest $request)
    {
        ...
        if ($request->hasFile('bg_image')) {
            ...
            $fileName = ImgUploader::UploadImage('company_bg_images', $image, $request->input('name'), 700, 200, false);
    ...
    public function editCompany($id)
    {
        ...
        if ($company->package_id > 0) {
            ...
            $packages = Package::select('id', DB::raw("CONCAT(`package_job`,', ' ,`package_title`, ', NPR', `package_price`, ', Days:', `package_num_days`, ', Listings:', `package_num_listings`) AS package_detail"))->where('package_for', 'like', 'employer')->where('id', '<>', $company->package_id)->where('package_price', '>=', $package->package_price)->pluck('package_detail', 'id')->toArray();
        } else {
            $packages = Package::select('id', DB::raw("CONCAT(`package_job`,', ' ,`package_title`, ', NPR', `package_price`, ', Days:', `package_num_days`, ', Listings:', `package_num_listings`) AS package_detail"))->where('package_for', 'like', 'employer')->pluck('package_detail', 'id')->toArray();
        }
    ...
    public function updateCompany($id, CompanyFormRequest $request)
    {
        ...
        if ($request->hasFile('bg_image')) {
            $is_deleted = $this->deleteCompanyBgImage($company->id);
            $image = $request->file('bg_image');
            $fileName = ImgUploader::UploadImage('company_bg_images', $image, $request->input('name'), 700, 200, false);
            $company->bg_image = $fileName;
        }
