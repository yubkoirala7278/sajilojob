...
class JobController extends Controller
{
    ...
    public function fetchJobsData(Request $request)
    {
        ...
        return Datatables::of($jobs)
                        ->filter(function ($query) use ($request) {
                            ...
                            if ($request->has('is_top') && $request->is_top != -1) {
                                $query->where('jobs.is_top', '=', "{$request->get('is_top')}");
                            }
                            if ($request->has('is_hot') && $request->is_hot != -1) {
                                $query->where('jobs.is_hot', '=', "{$request->get('is_hot')}");
                            }
                        ...
                        ->addColumn('action', function ($jobs) {
                            ...
                            $topTxt = 'Make Top';
                            $topHref = 'makeTop(' . $jobs->id . ');';
                            $topIcon = 'square-o';
                            if ((int) $jobs->is_top == 1) {
                                $topTxt = 'Make Not Top';
                                $topHref = 'makeNotTop(' . $jobs->id . ');';
                                $topIcon = 'check-square-o';
                            }

                            $hotTxt = 'Make Hot';
                            $hotHref = 'makeHot(' . $jobs->id . ');';
                            $hotIcon = 'square-o';
                            if ((int) $jobs->is_hot == 1) {
                                $hotTxt = 'Make Not Hot';
                                $hotHref = 'makeNotHot(' . $jobs->id . ');';
                                $hotIcon = 'check-square-o';
                            }
				...
				<div class="btn-group">
					...
					<ul class="dropdown-menu">
						...
						<li>
						<a href="javascript:void(0);" onClick="' . $topHref . '" id="onclickTop' . $jobs->id . '"><i class="fa fa-' . $topIcon . '" aria-hidden="true"></i>' . $topTxt . '</a>
						</li>
						<li>
						<a href="javascript:void(0);" onClick="' . $hotHref . '" id="onclickTop' . $jobs->id . '"><i class="fa fa-' . $hotIcon . '" aria-hidden="true"></i>' . $hotTxt . '</a>
						</li>
						<li>
    ...
    public function makeTopJob(Request $request)
    {
        $id = $request->input('id');
        try {
            $job = Job::findOrFail($id);
    ...
    public function makeNotTopJob(Request $request)
    {
        $id = $request->input('id');
        try {
            $job = Job::findOrFail($id);
    ...
    public function makeHotJob(Request $request)
    {
        $id = $request->input('id');
        try {
            $job = Job::findOrFail($id);
    ...
    public function makeNotHotJob(Request $request)
    {
        $id = $request->input('id');
        try {
            $job = Job::findOrFail($id);
