...

{
...

    {
        ...
        $job->specification = $request->input('specification');
        ...
        $job->expiry_date = Carbon::now()->addDays(10);
...

    {
        ...
        $job->is_top = $request->input('is_top');
        ...
        foreach($users as $user){
            $test = (new sendAlertUser($user->email, $user->first_name, $link, $company->name));
            dispatch($test);
        }
...

    {
        ...
        $company_package_id = $company->package_id;

        $type = Package::find($company_package_id);
        $type_name = $type->package_job;
        if($type_name == "Top Job"){
            $job->is_top = 1;
        }
        if($type_name == "Hot Job"){
            $job->is_hot = 1;
        }
        if($type_name == "Feature Job"){
            $job->is_feature = 1;
        }
